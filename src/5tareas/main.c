...
/*
 * Periodos de las tareas del sistema 5.b del TP2
 */
...
/*
 * Tiempo de respuesta de las tareas del sistema 5.b del TP2
 */
#define TASK1_WCET		1000
#define TASK2_WCET		1000
...
//        sprintf( cMessage, "%s - %u\n\r", pcTaskGetTaskName( NULL ), uxReleaseCount );
		...
		sprintf(cMessage, "S %s - %u - %u\n\r", pcTaskGetTaskName( NULL),
				uxReleaseCount, xTaskGetTickCount());
        ...
        /*
		 * Hace la periodicidad de la tarea
		 */
		vTaskDelayUntil( &pxPreviousWakeTime, task->period );

		/*
		 * Cuando vuelve de la llamada de la funcion vTaskDelayUntil
		 * se lanza nuevamente la tarea
		 */
		uxReleaseCount += 1;
	}
